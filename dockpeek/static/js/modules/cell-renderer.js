  ...
  if (shouldGroupPorts) {
    ...
    cell.innerHTML = portGroups.map(group => {
      if (group.isRange) {
        ...
        return `<div class="flex items-center mb-1">${rangeBadge}${arrowSvg}<small class="text-secondary">${group.startPort.container_port}-${group.endPort.container_port}</small></div>`;
      } else {
